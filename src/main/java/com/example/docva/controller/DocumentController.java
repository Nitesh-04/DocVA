...
public class DocumentController {
    ...
    public ResponseEntity<Document> uploadDocument(
        ...
        Document document = documentService.uploadDocument(fileName, fileLink, username);
        return ResponseEntity.status(HttpStatus.CREATED).body(document);
    }

    @GetMapping("/file")
    public List<Document> getDocumentsByFileName(
            @RequestParam("filename") String fileName) {
        return documentService.getDocumentsByOwnerAndFileName(username,fileName);
    }

    @GetMapping("/time")
    public List<Document> getDocumentsBetween(
